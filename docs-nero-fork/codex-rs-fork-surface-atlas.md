...
| Model catalog runtime overlay | Startup-local model metadata overlay without replacing bundled catalog or disabling refresh | `NERO-EXPOSED` config + `NERO-INTERNAL` catalog merge | `model_catalog_overlay_json`, `ModelsResponse`, `model/list` | `core/src/config/mod.rs`, `core/src/models_manager/manager.rs`, `core/src/thread_manager.rs`, `app-server/tests/suite/v2/model_list.rs` |
