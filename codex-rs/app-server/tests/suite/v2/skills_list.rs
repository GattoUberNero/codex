...
fn write_deny_all_skill(root: &TempDir, name: &str) -> Result<()> {
    let skill_dir = root.path().join("skills").join(name);
    std::fs::create_dir_all(&skill_dir)?;
    let content = format!(
...
#[tokio::test]
async fn skills_list_filters_nero_agent_visibility() -> Result<()> {
    let codex_home = TempDir::new()?;
    let cwd = TempDir::new()?;
    let extra_root = TempDir::new()?;
    write_skill(&extra_root, "visible-skill")?;
    write_deny_all_skill(&extra_root, "hidden-skill")?;
    ...
    assert!(
        data[0]
            .skills
            .iter()
            .any(|skill| skill.name == "visible-skill")
    );
    assert!(
        data[0]
            .skills
            .iter()
            .all(|skill| skill.name != "hidden-skill")
    );
    Ok(())
}
