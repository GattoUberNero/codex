...
impl App {
    ...
    async fn session_state_for_thread_read(
        ...
        session.session_source = thread.source.clone().into();
    ...
    async fn handle_event(
        ...
        match event {
            ...
            AppEvent::ApplyNeroAutoHotkey { thread_id, action } => {
                self.handle_nero_auto_hotkey_event(app_server, thread_id, action)
                    .await;
            }
            AppEvent::ClearUi => {
